...
def main():
	...
	with tf.Graph().as_default():
		...
		vgg16_mentee = VGG16Mentee()
                #vgg16_mentee = VGG16Mentor()
                """               
		mentee_conv2, mentor_conv3, logits_mentor, softmax_temp_mentor = vgg16_mentor.build(images_placeholder)

                mentee_conv1, mentee_conv2, logits_mentee, softmax_temp_mentee = vgg16_mentee.build(images_placeholder)
                embed = Embed()
                embed_loss_1, embed_loss_2 = embed.build(images_placeholder, mentee_conv3, mentor_conv5, mentor_conv1, mentee_conv2)

                embed_loss = tf.add(embed_loss_1, embed_loss_2)

                temp_softmax_loss = tf.sqrt(tf.reduce_mean(tf.square(tf.subtract(softmax_temp_mentee, softmax_temp_mentor))))
                """
		...
		train_op = vgg16_mentee.training(mentee_logits_loss, learning_rate)
		...
		try:
			for i in range(NUM_ITERATIONS):
				...
				feed_dict = fill_feed_dict(data_input_train, images_placeholder,
								labels_placeholder, sess)
				_, loss_value = sess.run([train_op, mentee_logits_loss], feed_dict=feed_dict)
				if i % 5 == 0:
