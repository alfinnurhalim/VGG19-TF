...
class Teacher(object):
	...
	def build(self, rgb, value, phase_train):
		...
		with tf.name_scope('teacher_fc1') as scope:
			...
			conv6_1_flat = tf.reshape(self.pool4, [-1, shape])
			fc1l = tf.nn.bias_add(tf.matmul(conv6_1_flat, fc1w), fc1b)
		...
		with tf.name_scope('teacher_fc3') as scope:
			...
			self.fc3l = tf.nn.bias_add(tf.matmul(self.fc2, fc3w), fc3b)
                        self.fc3l = tf.nn.relu(self.fc3l)
