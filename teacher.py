...
class VGG16(object):
	...
	def build(self, rgb, train_mode=None):
		...
		with tf.name_scope('conv1_1') as scope:
			kernel = tf.Variable(tf.truncated_normal([3,3,3,96], dtype = tf.float32, stddev = 1e-2), name='weights', trainable = True)
                        conv = tf.nn.conv2d(rgb, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(tf.constant(0.0, shape = [96], dtype = tf.float32), name='biases', trainable = True)
		...
		with tf.name_scope('conv2_1') as scope:
			kernel = tf.Variable(tf.truncated_normal([3,3,96,192]), name='weights', trainable = True)
			conv = tf.nn.conv2d(self.pool1, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(tf.constant(0.0, shape = [192], dtype = tf.float32), name='biases', trainable = True)
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('conv3_1') as scope:
			kernel = tf.Variable(tf.truncated_normal([3, 3, 192,192], dtype = tf.float32, stddev = 1e-2), name='weights', trainable = True)
		...
		with tf.name_scope('fc1') as scope:
			...
			fc1b = tf.Variable(tf.constant, name='biases', trainable = True)
