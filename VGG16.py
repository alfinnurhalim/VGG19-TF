...
class VGG16(object):
	...
	def build(self, rgb, train_mode=None):
		...
		with tf.name_scope('conv1_1') as scope:
			kernel = tf.Variable(self.data_dict["conv1_1"][0], name='weights')
                        conv = tf.nn.conv2d(rgb, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv1_1"][1], name='biases')
		...
		with tf.name_scope('conv1_2') as scope:
			kernel = tf.Variable(self.data_dict["conv1_2"][0], name='weights')
			conv = tf.nn.conv2d(self.conv1_1, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv1_2"][1], name='biases')
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('conv2_1') as scope:
			kernel = tf.Variable(self.data_dict["conv2_1"][0], name='weights')
			conv = tf.nn.conv2d(self.pool1, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv2_1"][1], name='biases')
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('conv2_2') as scope:
			kernel = tf.Variable(self.data_dict["conv2_2"][0], name='weights')
			conv = tf.nn.conv2d(self.conv2_1, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv2_2"][1], name='biases')
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('conv3_1') as scope:
			kernel = tf.Variable(self.data_dict["conv3_1"][0], name='weights')
			conv = tf.nn.conv2d(self.pool2, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv3_1"][1], name='biases')
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('conv3_2') as scope:
			kernel = tf.Variable(self.data_dict["conv3_2"][0], name='weights')
			conv = tf.nn.conv2d(self.conv3_1, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv3_2"][1], name='biases')
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('conv3_3') as scope:
			kernel = tf.Variable(self.data_dict["conv3_3"][0], name='weights')
			conv = tf.nn.conv2d(self.conv3_2, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv3_3"][1], name='biases')
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('conv4_1') as scope:
			kernel = tf.Variable(self.data_dict["conv4_1"][0], name='weights')
			conv = tf.nn.conv2d(self.pool3, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv4_1"][1], name='biases')
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('conv4_2') as scope:
			kernel = tf.Variable(self.data_dict["conv4_2"][0], name='weights')
			conv = tf.nn.conv2d(self.conv4_1, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv4_2"][1], name='biases')
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('conv4_3') as scope:
			kernel = tf.Variable(self.data_dict["conv4_3"][0], name='weights')
			conv = tf.nn.conv2d(self.conv4_2, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv4_3"][1], name='biases')
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('conv5_1') as scope:
			kernel = tf.Variable(self.data_dict["conv5_1"][0], name='weights')
			conv = tf.nn.conv2d(self.pool4, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv5_1"][1], name='biases')
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('conv5_2') as scope:
			kernel = tf.Variable(self.data_dict["conv5_2"][0], name='weights')
			conv = tf.nn.conv2d(self.conv5_1, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv5_2"][1], name='biases')
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('conv5_3') as scope:
			kernel = tf.Variable(self.data_dict["conv5_3"][0], name='weights')
			conv = tf.nn.conv2d(self.conv5_2, kernel, [1, 1, 1, 1], padding='SAME')
			biases = tf.Variable(self.data_dict["conv5_3"][1], name='biases')
			#out = tf.nn.bias_add(conv, biases)
                        #mean, var = tf.nn.moments(out, axes=[0])
                        #batch_norm = (out - mean) / tf.sqrt(var + tf.constant(1e-10))
		...
		with tf.name_scope('fc1') as scope:
			...
			fc1b = tf.Variable(self.data_dict["fc6"][1], name='biases')
		...
		with tf.name_scope('fc2') as scope:
			fc2w = tf.Variable(self.data_dict["fc7"][0], name='weights')
			fc2b = tf.Variable(self.data_dict["fc7"][1], name='biases')
