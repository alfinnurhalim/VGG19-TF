...
class FitNet(object):
    ...
    def main(self, _):
            ...
            #with tf.device(device):
                    ...
                    data_input_test = DataInputTest(dataset_path, FLAGS.test_dataset,FLAGS.batch_size)
                    ...
                    if FLAGS.teacher:
                        ...
                        train_op = optimizer.minimize(loss + data_dict_embed.loss_embed_5)
                        softmax = data_dict_student.softmax_output
                        init = tf.initialize_all_variables()
                        sess.run(init)
                        saver.restore(sess, FLAGS.teacher_weights_filename)
...
if __name__ == '__main__':
        ...
        '--test_dataset',
        type = str,
        default = "test_map.txt")

        parser.add_argument(
        '--train_dataset',
        type = str,
        default = "train_map.txt")
