...
class FitNet(object):
    ...
    def train_independent_teacher(self, sess):
            print("Teacher")
    ...
    def train_student_with_hint_based_training(self, sess):
    ...
    def train_student_with_knowledge_distillation(self, sess):
    ...
    def train_student_with_hard_logits(self, sess):
    ...
    def train_student_with_multiple_layers(self, sess):
            ...
            train_op = optimizer.minimize(loss + data_dict_embed.loss_embed_6 + data_dict_embed.loss_embed_7 + data_dict_embed.loss_embed_8)
            self.softmax = data_dict_student.softmax_output
            init = tf.initialize_all_variables()
            sess.run(init)
            saver.restore(sess, FLAGS.teacher_weights_filename)
    ...
    def train_student_with_new_method(self, sess):
            ...
            loss = student.loss(labels_placeholder)
            self.softmax = data_dict_student.softmax_output
            self.train_op_for_multiple_optimizers(lr, loss, data_dict_embed)
            init = tf.initialize_all_variables()
            sess.run(init)

    def train_independent_student(self, sess):
                ...
                for i in range(NUM_ITERATIONS):
                    ...
                    if FLAGS.student or FLAGS.teacher:
                        ...
                        if FLAGS.dependent_student and FLAGS.multiple_optimizers:
                            self.calculate_loss_with_multiple_optimizers(loss, data_dict_embed, sess, feed_dict)
                            if i % 10 == 0:
                                if FLAGS.multiple_optimizers_l1:
    ...
    def main(self, _):
            ...
            with tf.Graph().as_default():
                    ...
                    if FLAGS.teacher:
                        self.train_independent_teacher(sess)
                        
                    elif (FLAGS.student and FLAGS.HT):
                        self.train_student_with_hint_based_training(sess)

                    elif (FLAGS.student and FLAGS.KD):
                        self.train_student_with_knowledge_distillation(sess)

                    elif (FLAGS.student and FLAGS.hard_logits):
                        self.train_student_with_hard_logits(sess)

                    elif (FLAGS.student and FLAGS.multiple_layers):
                        self.train_student_with_multiple_layers(sess)

                    elif (FLAGS.dependent_student and FLAGS.new_method):
                        self.train_student_with_new_method(sess)

                    elif FLAGS.student:
                        self.train_independent_student(sess)
